...
(Bookkeeping note, app#149: this heading is correctly labeled. #126's own merge commit (`eea0623`) is the `v0.22.9` tag target and is the sole commit that ever set `package.json` to `0.22.9`; npm published `0.22.9` ~18 minutes later. The 0.22.10 window (`v0.22.9..v0.22.10`) contains only #129 — #119/#121/#123 were already shipped and already have their own headings above, at 0.22.6/0.22.7/0.22.8. Those three intermediate versions were never tagged or published individually — only `v0.22.5` and `v0.22.9` were — so npm's actual `0.22.9` tarball is the union of the 0.22.6–0.22.9 diffs even though each got its own changelog entry; this entry documents only the slice #126 itself added.)
