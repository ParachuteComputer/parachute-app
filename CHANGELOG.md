...
- **`/v/<vault>/n/<note>` takes a name or an id on both segments (#195, closes #194).** `resolveVaultRef` tries exact name, then exact id (`vaultIdFromUrl`, derived from the vault URL so it is identical on every device and survives a local `renameVault`), then case-folded either; name beats id at equal specificity, ties broken by id order. A splat route `/v/:vault/n/*`, ranked below the existing `:id` and `:id/edit` routes, takes a multi-segment note PATH — a hand-written link no longer has to be percent-encoded to resolve. `/v/<vault>` alone switches to that vault and lands on its notes. The route still grants nothing: an unresolvable or unconnected vault renders the not-connected state rather than borrowing the active vault's session, now checked BEFORE the note reference.
- **Pin the encoded edit-suffix note path (#197).** Test-only pin of the `/edit` tail on an encoded note path, plus the `App.tsx` handoff it exercises.
- **Strike normalizes across emphasis (#198).** `toggleStrikethrough` now hoists its `~~` markers outside the `StrongEmphasis` / `Emphasis` spans a ragged selection intersects, the same way `toggleBold` and `toggleItalic` already stepped outside `Strikethrough`. Toolbar command only — no stored document is rewritten on load or save.
- **A note deep link survives login (#199).** `/n/<id>` is the app's own share format, so it lands as often on someone with no vault connected as on someone with one; logged out it hard-bounced to `/` and DISCARDED the id. `withReturnTo` (`lib/vault/url.ts`) is the one place the param is written, sanitized through the existing `safeInternalRedirect`, and the front door forwards it into every affordance that STARTS a connect — the neutral "Sign in" link, the self-host side door, and the hub card's `beginOAuth`. The `/v/<vault>` not-connected card's "Connect a vault" carries the whole `/v/…` address, so the return re-enters that route.
- **`safeInternalRedirect` rejects control characters (#200).** A WHATWG `URL` parser strips embedded ASCII controls, so `/\t/evil.com` read as a safe in-app path here but resolved to `https://evil.com/` once something downstream parsed it as a URL. Tab, CR, LF and every other C0/DEL byte are now rejected before the existing checks; the sanitizer's other rules are untouched and an unusable value still falls back to `/`.
- **`/v` with no vault is not a note named `v` (#201).** The bare prefix matched no `/v` route, fell through to the `/:id` bookmark shim, and was read as a NOTE named `v` against whatever vault happened to be active — precisely the cross-vault ambiguity `/v/<vault>/n/<note>` was built to remove, reachable from the prefix that removes it. `/v` now replaces to `/vaults`. Also pins four combinations #194's table implies but nothing exercised: a case-folded vault id through the router, an id vault AND a multi-segment note path in one address, `/v/<vault>/` with a trailing slash, and `/v/<vault>/n` with nothing after it.
- **Bare `/n` redirects to `/notes`, not a note named `n` (#202).** The identical hole one segment over from #201: `/n` with nothing after it fell to the same `/:id` bookmark shim and resolved as a note literally named `n` in whatever vault was active. It now gets its own `<Navigate to="/notes" replace />` shim beside the `/v` one, a mirroring NAVIGATION.md row, and a regression test alongside the existing `/settings` and `/notes`-vs-`/:id` route-order guards. The hub's collision guard already lists `n` in `APP_TOP_LEVEL_SEGMENTS`, so no hub change is needed for this one.
- **Share links carry the vault's server slug, not a local rename (#203, fixes #191).** "Copy link" built `/v/<vault>/n/<id>` from `VaultRecord.name`, which `renameVault` lets drift to any device-local label — a device that relabelled its vault "Aaron's stuff" copied an address that resolved on exactly one device in the world. `vaultShareRef` now prefers the SERVER SLUG read back off the record's URL (`https://box.example/vault/aaron` → `aaron`), the only canonical server-side spelling that survives on the client. Not the vault id: equally rename-proof but derived from the whole URL including the host, so it differs between devices reaching the same vault by different spellings. `resolveVaultRef` gains a final slug pass so the emitted form also opens on a device that renamed its own copy; the name and id forms keep resolving exactly as before, ahead of it. For an un-renamed vault the slug IS the name, so the copied string is byte-identical to today's.
- **Focus handed to CodeMirror on the focus-mode entry transition (#205, fixes #175).** Entering focus mode on the edit route collapses the header card, so whatever held the caret — the path input, the tag input, the Focus button itself — was unmounted mid-transition and DOM focus fell onto `<body>`, leaving Escape with no handler to reach. #173 fixed this for the toolbar button only; the hand-off now sits on the `focusOn` false→true edge where every door in converges, and the per-trigger `onClick` patch it subsumes is dropped.
