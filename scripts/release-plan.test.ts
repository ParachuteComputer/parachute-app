 ...
 * here is one where a wrong answer costs something real. Ported alongside
 * release-plan.ts from parachute-hub (hub#790); rewritten against vitest
 * since this repo's tests are vitest, not `bun:test` (see CLAUDE.md).
 */
...
describe("distTagFor", () => {
  test("prerelease → rc, release → latest", () => {
    expect(distTagFor("0.22.9-rc.2")).toBe("rc");
    expect(distTagFor("0.22.9")).toBe("latest");
  });
});

describe("compareVersions", () => {
  test("orders patch versions", () => {
    expect(compareVersions("0.22.5", "0.22.4")).toBeGreaterThan(0);
    expect(compareVersions("0.22.4", "0.22.5")).toBeLessThan(0);
    expect(compareVersions("0.22.5", "0.22.5")).toBe(0);
  });

  test("orders rc chains numerically, not lexically", () => {
    // The lexical trap: "rc.10" < "rc.9" as strings.
    expect(compareVersions("0.22.5-rc.10", "0.22.5-rc.9")).toBeGreaterThan(0);
    expect(compareVersions("0.22.5-rc.5", "0.22.5-rc.6")).toBeLessThan(0);
  });

  test("a release sorts above its own prereleases", () => {
    expect(compareVersions("0.22.5", "0.22.5-rc.99")).toBeGreaterThan(0);
  });

  test("major/minor dominate the prerelease suffix", () => {
    expect(compareVersions("0.23.0-rc.1", "0.22.9")).toBeGreaterThan(0);
  });
});

describe("decidePublish", () => {
  test("a fresh version publishes", () => {
    const d = decidePublish("0.22.9-rc.2", {
      versionExists: false,
      currentDistTagVersion: "0.22.9-rc.1",
    });
  ...
  test("an already-published version is skipped — the idempotency guarantee", () => {
    ...
    const d = decidePublish("0.22.9-rc.1", { versionExists: true });
  ...
  test("REFUSES to move a dist-tag backwards — the parallel-merge hazard", () => {
    ...
    const d = decidePublish("0.22.5-rc.5", {
      versionExists: false,
      currentDistTagVersion: "0.22.5-rc.6",
    });
  ...
  test("rc and latest are tracked independently", () => {
    // Publishing 0.23.0-rc.1 compares against the `rc` tag, not `latest`, so a
    // newer stable doesn't block the next prerelease line.
    const d = decidePublish("0.23.0-rc.1", {
      versionExists: false,
      currentDistTagVersion: "0.22.9-rc.5",
    });
    expect(d).toMatchObject({ publish: true });
  });
...
describe("readRegistry", () => {
  ...
  test("reads existence + the relevant dist-tag", async () => {
    const v = await readRegistry("@openparachute/app", "0.22.9-rc.2", (() =>
      json({
        versions: { "0.22.9-rc.1": {}, "0.22.9-rc.2": {} },
        "dist-tags": { rc: "0.22.9-rc.2", latest: "0.22.8" },
      })) as unknown as typeof fetch);
    expect(v).toMatchObject({ versionExists: true, currentDistTagVersion: "0.22.9-rc.2" });
  });

  test("picks the dist-tag matching the version's channel", async () => {
    const v = await readRegistry("@openparachute/app", "0.22.9", (() =>
      json({
        versions: {},
        "dist-tags": { rc: "0.22.9-rc.2", latest: "0.22.8" },
      })) as unknown as typeof fetch);
  ...
  test("a 5xx is ambiguous", async () => {
    const v = await readRegistry("@openparachute/app", "1.0.0", (() =>
