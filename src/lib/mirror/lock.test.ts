...
import { mirrorLockName, underMirrorVaultLock, withMirrorVaultLock } from "./lock";
...
describe("underMirrorVaultLock (user-initiated: wait for the holder)", () => {
  it("acquires the SAME per-vault name WITHOUT ifAvailable, so it queues", async () => {
    ...
    expect(locks.requests[0]?.options).toBeUndefined();
  });
