...
// (app.parachute.computer / *.pages.dev), not under a `/notes/` sub-mount, so
// the default base is `/`. The runtime mount detector still lets the same
// bundle serve under a sub-path if an operator ever mounts it there.
const basePath = normalizeBase(process.env.VITE_BASE_PATH ?? "/");

const DISPLAY_NAME = "Parachute";
const TAGLINE = "The super-surface for your Parachute";
